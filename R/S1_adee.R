...
# Required packages
required_packages <- c(
  "dplyr", "ggplot2", "survival", "survminer",
...
p2 <- ggplot(adee_pfs, aes(x = EXPOSURE_VAR, fill = EXPOSURE_CAT)) +
  ...
  labs(
    title = "Exposure Distribution by Tertile",
...
p3 <- ggplot(adee_pfs, aes(x = EXPOSURE_VAR, y = AVAL, color = factor(EVENT))) +
  ...
  labs(
    title = "Raw Exposure-Response Relationship",
    x = "Exposure (AUC, μg·h/mL)",
...
p4 <- ggplot(hr_data, aes(x = estimate, y = term)) +
  ...
  scale_x_log10() +
  labs(
    title = "Hazard Ratios from Multivariable Cox Model",
...
pred_data$median_tte <- predict(weibull_model,
  newdata = pred_data,
  type = "quantile", p = 0.5
)

# Predict 25th and 75th percentiles for uncertainty bands
pred_data$q25_tte <- predict(weibull_model,
  newdata = pred_data,
  type = "quantile", p = 0.25
)
pred_data$q75_tte <- predict(weibull_model,
  newdata = pred_data,
...
p5 <- ggplot() +
  ...
  geom_point(
    data = adee_pfs %>% filter(EVENT == 1),
    aes(x = EXPOSURE_VAR, y = AVAL, size = WTBL),
    alpha = 0.4, color = "red"
  ) +
  # Censored points
  geom_point(
    data = adee_pfs %>% filter(EVENT == 0),
...
p7 <- ggplot() +
  geom_line(
    data = subgroup_predictions,
    aes(x = EXPOSURE_VAR, y = median_tte, color = WT_GROUP),
    linewidth = 1.2
  ) +
  geom_point(
    data = adee_pfs_subgroup %>% filter(EVENT == 1),
