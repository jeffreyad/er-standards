...
While CDISC has developed standards for pharmacokinetic data—including the Study Data Tabulation Model Implementation Guide for Pharmacokinetics (SDTM-PK) [@cdisc_sdtmpk] and the Basic Data Structure for ADaM PopPK [@cdisc_bdspoppk]—no equivalent standards exist specifically for ER analyses. This gap creates challenges in reproducibility, efficiency, quality control, and regulatory review.

## CDISC PK Standards as a Foundation

The SDTM-PK and BDS for PopPK standards provide valuable principles applicable to ER analyses:

**From SDTM-PK**:
- Relative time variables (PCTPT, PCELTM)
- Standardized parameter codes
- Clear traceability through date/time variables
- 8-character variable name compliance

**From BDS for PopPK**:
- Parameter-based structure (one record per subject per parameter)
