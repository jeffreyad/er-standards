...
The latest version of {admiral} 1.4 includes some new experimental functions to facilitate PK analysis including `derive_var_nfrlt()` for deriving nominal time `NFRLT` "Nominal Relative Time for First Dose" which uses `convert_xxtpt_to_hours()` which uses regular expressions to convert `PCTPT` or other SDTM timepoints to numeric hours.
...
: Exposure metrics {#tbl-trad-exposure}
...
Exposure-response data standards were developed following FDA guidance 
[@fda2003er] and CDISC ADaM IG v1.3 [@cdisc2021adam]. Example datasets 
(ADEE, ADES, ADTR) were implemented using admiral [@admiral2024] and 
admiralonco [@admiralonco2024] R packages.
